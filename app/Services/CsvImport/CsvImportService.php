    ...
    public function importCsv(
        ...
        try {
            ...
            while (($row = fgetcsv($file, 0, $delimiter)) !== false) {
                ...
                try {
                    // Cas spécial pour les clients - nous devons nous assurer qu'ils ont un contact primaire
                    if ($tableName === 'clients') {
                        // Vérifier si un external_id est défini, sinon en générer un
                        if (!isset($data['external_id'])) {
                            $data['external_id'] = Uuid::uuid4()->toString();
                        }
                        
                        // Insérer le client
                        $clientId = DB::table($tableName)->insertGetId($data);
                        
                        // Créer un contact primaire si nécessaire
                        $contactData = [
                        ...
                        // Transférer les champs de contact du client vers le contact si présents
                        $contactFields = ['name', 'email', 'primary_number', 'secondary_number'];
                        ...
                        // Si aucun nom n'est défini, utiliser le nom de l'entreprise
                        if (!isset($contactData['name']) && isset($data['company_name'])) {
                            $contactData['name'] = $data['company_name'] . ' (Contact)';
                        }
